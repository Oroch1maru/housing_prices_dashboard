...
def prepare_data(cls, input_data_path: Path) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
...
def train(X_train: pd.DataFrame, y_train: np.ndarray) -> RandomForestRegressor:
...
def predict(X: pd.DataFrame, model: RandomForestRegressor) -> np.ndarray:
    Y = model.predict(X)
    return Y

def save_model(model: RandomForestRegressor, filename: Path) -> None:
    with open(filename, 'wb'):
        joblib.dump(model, filename, compress=3)

def load_model(filename: Path) -> RandomForestRegressor:
